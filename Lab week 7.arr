...
# Checking if the bandwith is greater than the sum of the bandwitdth of the left and the right.
fun fits-capacities(k :: SensorNet) -> Boolean:
  cases(SensorNet) k:
    | sensor(s) => true
    | hub(b, l, r) => 
...
fun deepest-depth(k :: SensorNet) -> Number:
  doc: "The function returns the depth of the deepest sensor in a given SensorNet"
  cases(SensorNet) k:
    | sensor(s) => 0
...
fun apply-scale(k :: SensorNet, s :: Number) -> SensorNet:
  doc: "Divides the bandwidth of a SensorNet object by s"
  cases(SensorNet) k block:
    | sensor(w) => sensor(w / s)
    | hub(b, l, r) => 
      hub(b / s,
        apply-scale(l, s),
...
fun needed-scale(k :: SensorNet) -> Number:
  cases (SensorNet) k:
    | sensor(rate)  => 1
    | hub(bw, l, r) =>
      block:
...
fun scale-to-fit(k :: SensorNet) -> SensorNet:
